...
class Parser {
   ...
   */
  parseColGroup(parent: HTMLElement): Column[] {
   ...
   * @returns The parsed columns.
   */
  parseSummaryRow(parent: HTMLElement, columns: Column[]) {
  ...
  parseRaceScore(element: HTMLElement) {
   ...
   * @returns The parsed columns.
   */
  parseSummaryRows(parent: HTMLElement, columns: Column[]) {
    const boats = [];
    for (const node of parent.children) {
      // Skip any unexpected nodes.
      ...
      boats.push(this.parseSummaryRow(node as HTMLElement, columns));
    }
    return boats;
  }

  parseTable(tableElement: Element) {
    let columns: Column[] = [];
    let boats;
    ...
    for (const node of tableElement.children) {
      switch (node.nodeName) {
        case 'COLGROUP':
          columns = this.parseColGroup(node as HTMLElement);
          break;
        // case 'THEAD':
        // Don't bother looking at the headings.
        // break;
        case 'TBODY':
          boats = this.parseSummaryRows(node as HTMLElement, columns);
          break;
      }
    }
  ...
  parseScoredGroup(titleElement: Element): ScoredGroup | false {
    ...
    do {
      ...
      if (el.classList.contains('summarycaption')) {
        ...
        partialGroup.table = this.parseTable(el);
      }
  ...
  parse(doc: Document) {
    ...
    scoredGroupTitleElements.forEach((el) => {
      const group = this.parseScoredGroup(el);
      if (group) {
